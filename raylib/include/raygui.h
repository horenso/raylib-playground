 ...
 * be disabled with the config flag RAYGUI_STANDALONE. In that case is up to the user to provide another backend to
 * cover library needs
 *
...
#define GUI_BUTTON_DOWN_ALT                                                                                            \
    (IsMouseButtonDown(MOUSE_RIGHT_BUTTON) || IsGamepadButtonDown(0, GAMEPAD_BUTTON_RIGHT_FACE_RIGHT))
#endif
#if !defined(GUI_BUTTON_PRESSED)
#define GUI_BUTTON_PRESSED                                                                                             \
    (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) || IsGamepadButtonPressed(0, GAMEPAD_BUTTON_RIGHT_FACE_DOWN))
#endif
#if !defined(GUI_BUTTON_PRESSED_MID)
#define GUI_BUTTON_PRESSED_MID                                                                                         \
    (IsMouseButtonPressed(MOUSE_MIDDLE_BUTTON) || IsGamepadButtonPressed(0, GAMEPAD_BUTTON_RIGHT_FACE_UP))
#endif
#if !defined(GUI_BUTTON_RELEASED)
#define GUI_BUTTON_RELEASED                                                                                            \
    (IsMouseButtonReleased(MOUSE_LEFT_BUTTON) || IsGamepadButtonReleased(0, GAMEPAD_BUTTON_RIGHT_FACE_DOWN))
#endif
...
typedef enum {
    ...
    TEXT_ALIGNMENT_VERTICAL =
        21, // Text vertical alignment inside text bounds (after border and padding): 0-Top, 1-Middle, 2-Bottom
...
RAYGUIAPI int GuiTabBarEx(Rectangle bounds, char **text, int count, int *hscroll, int *active,
...
RAYGUIAPI int GuiColorPickerHSV(
    ...
    Vector3 *colorHsv); // Color Picker control, using Hue-Saturation-Value color data, includes Color bar controls
...
#define CHECK_BOUNDS_ID(src, dst)                                                                                      \
    (((int)src.x == (int)dst.x) && ((int)src.y == (int)dst.y) && ((int)src.width == (int)dst.width) &&                 \
...
static int GuiFontIconBaking(Image *imFont, Font font,
        ...
        if (editMode) {
            ...
            if ((textBoxCursorIndex > 0) && GUI_KEY_PRESSED(KEY_LEFT) &&
                ...
                textBoxCursorIndex = offset;
                       ...
                       (GUI_KEY_PRESSED(KEY_LEFT) || (GUI_KEY_DOWN(KEY_LEFT) && autoCursorShouldTrigger))) {
                int prevCodepointSize = 0;
                GetCodepointPrevious(text + textBoxCursorIndex, &prevCodepointSize);
...
int GuiSpinner(Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode) {
    ...
    Rectangle valueBoxBounds = {
        ...
        bounds.y,
        bounds.width -
            2 * (GuiGetStyle(VALUEBOX, SPINNER_BUTTON_WIDTH) + GuiGetStyle(VALUEBOX, SPINNER_BUTTON_SPACING)),
        bounds.height};
    ...
    if (state == STATE_DISABLED) {
        ...
        if (*value >= maxValue) {
            ...
            GuiDrawRectangle(
                ...
                0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(
                RAYGUI_CLITERAL(Rectangle){bounds.x + bounds.width - (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
                                           bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
                                           bounds.height + GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) - 1},
                0, BLANK, GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
        }
...
int GuiColorBarHue(Rectangle bounds, const char *text, float *hue) {
    ...
    if (state != STATE_DISABLED) {
        ...
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y, bounds.width, bounds.height / 6.0f},
                                ...
                                Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha));
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + 1 * (bounds.height / 6.0f),
                                                           bounds.width, bounds.height / 6.0f},
                                Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 0, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 0, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){255, 255, 0, 255}, guiAlpha));
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + 2 * (bounds.height / 6.0f),
                                                           bounds.width, bounds.height / 6.0f},
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 0, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 255, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 255, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 0, 255}, guiAlpha));
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + 3 * (bounds.height / 6.0f),
                                                           bounds.width, bounds.height / 6.0f},
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 255, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){0, 0, 255, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){0, 0, 255, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){0, 255, 255, 255}, guiAlpha));
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + 4 * (bounds.height / 6.0f),
                                                           bounds.width, bounds.height / 6.0f},
                                Fade(RAYGUI_CLITERAL(Color){0, 0, 255, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){255, 0, 255, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){255, 0, 255, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){0, 0, 255, 255}, guiAlpha));
        DrawRectangleGradientEx(RAYGUI_CLITERAL(Rectangle){bounds.x, bounds.y + 5 * (bounds.height / 6.0f),
                                                           bounds.width, bounds.height / 6.0f},
                                Fade(RAYGUI_CLITERAL(Color){255, 0, 255, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha),
                                Fade(RAYGUI_CLITERAL(Color){255, 0, 0, 255}, guiAlpha),
...
int GuiColorPanelHSV(Rectangle bounds, const char *text, Vector3 *colorHsv) {
    ...
    if ((prevColorHsv.x != colorHsv->x) || (prevColorHsv.y != colorHsv->y) || (prevColorHsv.z != colorHsv->z)) {
        result = RESULT_CHANGED;
    }
    //--------------------------------------------------------------------
...
void GuiDrawIcon(int iconId, int posX, int posY, int pixelSize, Color color) {
    if ((guiIconFontOffsetY > 0) && (iconId < RAYGUI_ICON_MAX_FONT_BACKED)) {
        ...
        Rectangle dstRec = {(float)posX, (float)posY, (float)pixelSize * RAYGUI_ICON_SIZE,
...
#ifndef TEXT_VALIGN_PIXEL_OFFSET
...
static int GuiFontIconBaking(Image *imFont, Font font, Rectangle *whiteRec) {
    ...
    if ((maxGlyphRecY + reqHeight) > imFont->height) {
        ...
        for (int y = 0; y < 4; y++) {
            for (int x = 0; x < 4; x++) {
                ((unsigned short *)newImData)[(imFont->height - y - 1) * imFont->width + imFont->width - x - 1] =
                    0x0000;
            }
        }
